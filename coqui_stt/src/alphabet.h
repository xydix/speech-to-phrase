...
public:
  ...
  // Serialize alphabet into a text representation (ie. config file read by
  // `init`)
  ...
  bool IsSpace(unsigned int index) const { return index == space_index_; }

  unsigned int GetSpaceLabel() const { return space_index_; }

  unsigned int GetBlankLabel() { return this->GetSize(); }
