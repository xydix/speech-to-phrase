...
std::istream &getline_crossplatform(std::istream &is, std::string &t) {
  t.clear();
...
int Alphabet::init(const char *config_file) {
...
void Alphabet::InitFromLabels(const std::vector<std::string> &labels) {
  space_index_ = -2;
...
std::string Alphabet::SerializeText() {
  ...
  out << "# Each line in this file represents the Unicode codepoint (UTF-8 "
         "encoded)\n"
      << "# associated with a numeric index.\n"
      << "# A line that starts with # is a comment. You can escape it with \\# "
         "if you wish\n"
...
int Alphabet::Deserialize(const char *buffer, const int buffer_size) {
...
size_t Alphabet::GetSize() const { return entrySize(); }

std::vector<std::string> Alphabet::GetLabels() const {
...
bool Alphabet::CanEncodeSingle(const std::string &input) const {
  return contains(input);
}

bool Alphabet::CanEncode(const std::string &input) const {
...
std::string Alphabet::DecodeSingle(unsigned int label) const {
...
std::string Alphabet::Decode(const unsigned int *input, int length) const {
  std::string word;
...
bool UTF8Alphabet::CanEncodeSingle(const std::string &input) const {
  return true;
}

bool UTF8Alphabet::CanEncode(const std::string &input) const { return true; }

std::vector<unsigned int> UTF8Alphabet::Encode(const std::string &input) const {
  std::vector<unsigned int> result;
